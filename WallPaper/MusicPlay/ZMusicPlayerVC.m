...
//  iOSProduct
//
//  Created by Never on 2016/12/30.
//  Copyright © 2016年 oftenfull_iOSTeam. All rights reserved.
//
/*
 *  需要在info.plist中申请后台播放权限 
 *   Required background modes(NSArray)-->(item)App plays audio or streams audio/video using AirPlay
 */
...
- (void)animation{
    ...
    [self.musicIcon.layer addAnimation:animation forKey:nil];
    
    _musicAnimateView = [HeeeMusicAnimateView new];
    _musicAnimateView.viewColor = [UIColor colorWithRed:28/255.0 green:184/255.0 blue:36/255.0 alpha:1.0];
    _musicAnimateView.totalWidth = 50;
    _musicAnimateView.center = CGPointMake(100, 120);
...
-(void)reloadUI:(MusicModel*)model
{
    ...
    [self.musicIcon sd_setImageWithURL:[NSURL URLWithString:model.cover] placeholderImage:[UIImage imageNamed:@"minion"]];
